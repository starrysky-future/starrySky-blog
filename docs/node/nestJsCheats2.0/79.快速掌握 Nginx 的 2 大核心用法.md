...
![](images/image-2383.jpg)

输入容器名和要映射的端口：

![](images/image-2384.jpg)

这里把宿主机的 81 端口映射到容器内的 80 端口，点击 run。

这时候就可以看到 docker 容器跑起来了，并且打印了日志：

![](images/image-2385.jpg)

浏览器访问下 http://localhost:81 可以看到 nginx 欢迎页面：

![](images/image-2386.jpg)

这很明显是容器里跑的服务。

![](images/image-2387.jpg)
...
![](images/image-2388.jpg)

里面的 /usr/share/nginx/html/ 目录下面就是所有的静态文件。

双击点开 index.html 看看：

![](images/image-2389.jpg)

![](images/image-2390.jpg)
...
![](images/image-2391.jpg)

docker cp 这个命令就是用于在宿主机和容器之间复制文件和目录的。

![](images/image-2392.jpg)
...
![](images/image-2394.jpg)

但当目标目录存在的时候，docker 会把他复制到目标目录下面：

![](images/image-2395.jpg)

我们需要先删除容器的这个目录，再复制：

![](images/image-2396.jpg)
...
![](images/image-2397.jpg)

然后浏览器访问下试试：

![](images/image-2398.jpg)

![](images/image-2399.jpg)
...
![](images/image-2404.jpg)

然后来看第一条路由：

![](images/image-2405.jpg)

location 和路径之间加了个 =，代表精准匹配，也就是只有完全相同的 url 才会匹配这个路由。

![](images/image-2406.jpg)

![](images/image-2407.jpg)

不带 = 代表根据前缀匹配，后面可以是任意路径。

![](images/image-2408.jpg)

这里的 $uri 是取当前路径。

![](images/image-2409.jpg)

![](images/image-2410.jpg)

然后如果想支持正则，就可以加个 ~。

![](images/image-2411.jpg)

这里的正则语法不难看懂，就是 /333/bbb 开头，然后中间是任意字符，最后 .html 结尾的 url。

![](images/image-2412.jpg)

![](images/image-2413.jpg)

但是它是区分大小写的，比如这样就不行了：

![](images/image-2414.jpg)

换成小写就可以：

![](images/image-2415.jpg)

如果想让正则不区分大小写，可以再加个 \*

![](images/image-2416.jpg)

试一下：

![](images/image-2417.jpg)

![](images/image-2418.jpg)
...
![](images/image-2419.jpg)

这时候就有两个 /444 的路由了：

![](images/image-2420.jpg)

这时候浏览器访问，还是匹配上面的那个路由：

![](images/image-2421.jpg)
...
![](images/image-2422.jpg)

这时候同一个 url，匹配的就是下面的路由了：

![](images/image-2423.jpg)
...
![](images/image-2425.jpg)

![](images/image-2426.jpg)

![](images/image-2427.jpg)

前面用过 root：

![](images/image-2428.jpg)
...
![](images/image-2429.jpg)

![](images/image-2430.jpg)
...
![](images/image-2432.jpg)

浏览器就访问 http://localhost:3000 看到 hello world 就代表 nest 服务跑成功了：

![](images/image-2433.jpg)

添加一个全局的前缀 /api

![](images/image-2434.jpg)

![](images/image-2435.jpg)
...
![](images/image-2437.jpg)

也就是这样的：

![](images/image-2438.jpg)
...
![](images/image-2441.jpg)

然后浏览器访问下。

直接访问 nest 服务的话，是没有这个 header 的：

![](images/image-2442.jpg)

![](images/image-2443.jpg)

访问 nginx 的反向代理服务器，做一次中转：

![](images/image-2444.jpg)

![](images/image-2445.jpg)
...
![](images/image-2446.jpg)

把 nest 服务停掉，然后重新 npm run start

![](images/image-2447.jpg)

3001 和 3002 端口各跑一个：

![](images/image-2448.jpg)

浏览器访问下，都是正常的：

![](images/image-2449.jpg)

![](images/image-2450.jpg)

控制台也打印了访问日志：

![](images/image-2451.jpg)

问题来了，现在有一个 nginx 服务器，两个 nest 服务器了，nginx 该如何应对呢？

![](images/image-2452.jpg)

nginx 的解决方式就是负载均衡，把请求按照一定的规则分到不同的服务器。

改下 nginx 配置文件：

![](images/image-2453.jpg)
...
![](images/image-2455.jpg)

可以看到两个 nest 服务，一个 3 次，一个 2 次。

![](images/image-2456.jpg)
...
![](images/image-2459.jpg)

然后我访问了 8 次 http://localhost:81/api

看打印的日志来看，差不多就是 2:1 的轮询几率。

![](images/image-2460.jpg)

这就是带权重的轮询。

我们再试下 ip_hash 的方式；

![](images/image-2461.jpg)
