...
![](images/image-3564.jpg)

这时候访问 http://localhost:3000 ，可以看到响应格式变了：

![](images/image-3565.jpg)
...
![](images/image-3566.jpg)

然后再试下其它接口：

响应格式确实变了：

![](images/image-3567.jpg)

抛出的异常还是由内置的 Exception Filter 来处理，返回响应：

![](images/image-3568.jpg)
...
![](images/image-3570.jpg)

试一下：

![](images/image-3571.jpg)

访问管理员的登录的接口。

可以看到控制台打印了请求的路径、请求方法、controller、handler 等信息：

![](images/image-3572.jpg)
...
![](images/image-3584.jpg)

现在返回的内容就是合理的了。

然后实现修改密码的接口：

![](images/image-3585.jpg)

![](images/image-3586.jpg)
...
![](images/image-3587.jpg)

登录用户端，拿到 access_token，然后访问 /user/update_password 和 /user/admin/update_password 接口：

![](images/image-3588.jpg)

![](images/image-3589.jpg)

![](images/image-3590.jpg)

![](images/image-3591.jpg)
...
![](images/image-3592.jpg)

数据库中可以查到 lisi 的邮箱：

![](images/image-3593.jpg)

带上 access_token 访问更新密码接口：

![](images/image-3594.jpg)

先手动去 redis 里添加 update_password_captcha_yy@yy.com 的 key，值为 123123（注意，我们现在用的是 redis 的 db1）

![](images/image-3595.jpg)

半小时过期。

![](images/image-3596.jpg)

然后再试下：

![](images/image-3597.jpg)

修改成功。

数据库里看不到具体的密码，但也能看出确实变了：

![](images/image-3598.jpg)
...
![](images/image-3600.jpg)

用之前的密码登录，会提示密码错误。

换成新密码就好了：

![](images/image-3601.jpg)
