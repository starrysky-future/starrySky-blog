...
![](images/image-3610.jpg)

但是失败的响应是这样：

![](images/image-3611.jpg)
...
![](images/image-3616.jpg)

然后把 LoginGuard 里的异常改成 UnLoginException

![](images/image-3617.jpg)

测试下：

![](images/image-3618.jpg)

现在当你不带 token 访问 /aaa 的时候，返回的就是自己定义的格式了。

搜索下之前代码里抛的异常：

![](images/image-3619.jpg)
...
![](images/image-3621.jpg)

这样，所有的代码都不用修改，返回的响应就已经变了：

![](images/image-3622.jpg)

![](images/image-3623.jpg)

所以，如果你只是想修改默认的响应格式，直接定义个 catch HttpException 的 filter 就好了。

但这样其实还有个问题：

![](images/image-3624.jpg)
...
![](images/image-3625.jpg)

在 postman 里再次请求这个接口，代码会在断点处断住：

![](images/image-3626.jpg)

可以看到，这时候 message 并不是具体的错误，具体的错误在 response.message 里：

![](images/image-3627.jpg)

所以我们可以这样改：

![](images/image-3628.jpg)
...
![](images/image-3629.jpg)

这样，错误显示就对了。

然后我们实现冻结用户的接口，冻结的用户不能预定会议室。

![](images/image-3630.jpg)

这个接口非常简单，就是修改一个字段：

![](images/image-3631.jpg)
...
![](images/image-3632.jpg)

![](images/image-3633.jpg)
...
![](images/image-3638.jpg)

![](images/image-3639.jpg)
...
![](images/image-3640.jpg)

没有传 pageNo 的时候设置为 1，没有传 pageSize 的时候设置为 2。

测试下：

![](images/image-3641.jpg)

现在传入 pageNo 和 pageSize 就可以查询出对应的数据，还有总条数：

![](images/image-3642.jpg)

服务端打印了两条 sql：

![](images/image-3643.jpg)
...
![](images/image-3646.jpg)

当我传 nickName 包含“里”的时候，服务端查询到数据只有一条。

![](images/image-3647.jpg)
