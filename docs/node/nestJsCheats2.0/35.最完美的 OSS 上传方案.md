...
![](images/image-954.jpg)

我买了 40G 的 OSS 国内通用资源包，花了 5 块钱。

然后我们创建个 Bucket（桶）：

![](images/image-955.jpg)
...
![](images/image-966.jpg)

这里的 accessKeyId 和 acessKeySecret 是什么呢？

本来我们身份认证都是通过用户名密码：

![](images/image-967.jpg)

但这样不够安全，所以我们创建了 accessKey 用来代表身份，用它来做身份认证，就算泄漏了，也不影响别的：

![](images/image-968.jpg)

我们创建个 accesKey：

![](images/image-969.jpg)

![](images/image-970.jpg)

创建完成后，拿到 accesKeyId 和 accessKeySecret 后运行代码：

![](images/image-971.jpg)

这里的 mao.png 是这样的：

![](images/image-972.jpg)

在控制台可以看到上传成功了：

![](images/image-973.jpg)
...
![](images/image-974.jpg)

让我们不要直接用 accessKey，而是创建一个子用户再创建 accessKey。

那我们就创建个子用户：

![](images/image-975.jpg)

![](images/image-976.jpg)

然后用这个 accessKey 的 id 和 secret 就好了：

![](images/image-977.jpg)

但你直接换上它还不行：

![](images/image-978.jpg)

会提示你 403，没有权限。

需要你授权下：

![](images/image-979.jpg)

新增一个授权：

![](images/image-980.jpg)

把 OSS 的管理和读取权限给这个子用户：

![](images/image-981.jpg)

然后再试下：

![](images/image-982.jpg)
...
![](images/image-983.jpg)

那万一泄漏了不就完蛋了么？

但是如果创建个 accessKey 用它来做身份认证：

![](images/image-984.jpg)

就算泄漏了我也可以禁用啊：

![](images/image-985.jpg)
...
![](images/image-987.jpg)

具体怎么用直接看文档就好了，这里我们就不试了：

![](images/image-988.jpg)
...
![](images/image-992.jpg)

这些代码不用记，文档里都有：

![](images/image-993.jpg)
...
![](images/image-994.jpg)

这时候你上传文件的时候会提示跨域错误：

![](images/image-995.jpg)

我们在控制台开启下跨域：

![](images/image-996.jpg)

然后再试下：

![](images/image-997.jpg)

上传成功了！

控制台文件列表也可以看到这个文件：

![](images/image-998.jpg)
