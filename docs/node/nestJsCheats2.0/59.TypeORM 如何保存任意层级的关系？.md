...
![](images/image-1708.jpg)

新闻网站的新闻分类：

![](images/image-1709.jpg)
...
![](images/image-1714.jpg)

我们在 mysql workbench 里看下：

![](images/image-1715.jpg)

![](images/image-1716.jpg)

可以看到 parentId 引用了自身的 id。

并且还有个 city_closure 表：

![](images/image-1717.jpg)

![](images/image-1718.jpg)

![](images/image-1719.jpg)

两个外键都引用了 city 表的 id。

先不着急解释为什么是这样的，我们插入一些数据试试：

![](images/image-1720.jpg)
...
![](images/image-1721.jpg)

可以看到数据插入成功了，并且返回了树形结构的结果。

在 mysql workbench 里看下：

![](images/image-1722.jpg)

在 city 表里保存着 city 记录之间的父子关系，通过 parentId 关联。

![](images/image-1723.jpg)

在 city_closure 表里记录了也记录了父子关系。

把插入数据的代码注释掉：

![image.png](images/image.png-1724.jpg)
...
![](images/image-1729.jpg)

这里换成 findAncestors、findDescendants 就是用扁平结构返回：
![](images/image-1730.jpg)

![](images/image-1731.jpg)

把 findTrees 换成 find 也是会返回扁平的结构：

![](images/image-1732.jpg)
...
![](images/image-1733.jpg)

这些 api 都是很实用的。

回过头来，再看下 @Tree 的 entity：

![](images/image-1734.jpg)
...
![](images/image-1735.jpg)

我们一般都是用 closure-table，或者 materialized-path。

其余两种有点问题：

![](images/image-1736.jpg)

![](images/image-1737.jpg)

把两个表删掉：

![](images/image-1738.jpg)

改成 materialized-path 重新跑：

![](images/image-1739.jpg)

可以看到，现在只生成了一个表：

![](images/image-1740.jpg)
...
![](images/image-1741.jpg)

可以看到，它通过 mpath 路径存储了当前节点的访问路径，从而实现了父子关系的记录：

![](images/image-1742.jpg)
