...
![](images/image-652.jpg)

服务端报错时返回 500：

![](images/image-653.jpg)

参数的错误返回 400：

![](images/image-654.jpg)
...
![](images/image-659.jpg)

这时候刷新页面，返回的就是 400 对应的响应：

![](images/image-660.jpg)

这个响应的格式是内置的 Exception Filter 生成的。

当然，你也可以直接抛具体的异常：

![](images/image-661.jpg)

![](images/image-662.jpg)
...
![](images/image-663.jpg)

![](images/image-664.jpg)
...
![](images/image-666.jpg)

![](images/image-667.jpg)

然后新建个调试配置文件：

![](images/image-668.jpg)

输入调试配置：

![](images/image-669.jpg)
...
![](images/image-670.jpg)

刷新页面，代码会在断点处断住：

![](images/image-671.jpg)

我们只要根据异常信息返回对应的响应就可以了：

![](images/image-672.jpg)
...
![](images/image-673.jpg)

但我们只是 @Catch 了 BadRequestException

如果抛的是其他异常，依然是原来的格式：

![](images/image-674.jpg)

比如我抛一个 BadGatewayException。

![](images/image-675.jpg)
...
![](images/image-676.jpg)

试一下：

![](images/image-677.jpg)

![](images/image-678.jpg)
...
![](images/image-680.jpg)

可以看到，提示的错误也不对了。

因为我们自定义的 exception filter 会拦截所有 HttpException，但是没有对这种情况做支持。

![](images/image-681.jpg)

先不加这个 filter。

这时候响应是这样的：

![](images/image-682.jpg)

我们对这种情况做下支持：

![](images/image-683.jpg)

启用自定义的 filter，然后打个断点：

![](images/image-684.jpg)

再次访问会在断点处断住：

![](images/image-685.jpg)

可以看到 ValidationPipe 的 response 格式是这样的。

所以我们可以这样改：

![](images/image-686.jpg)
...
![](images/image-687.jpg)

![](images/image-688.jpg)
...
![](images/image-695.jpg)

浏览器里访问下：

![](images/image-696.jpg)
