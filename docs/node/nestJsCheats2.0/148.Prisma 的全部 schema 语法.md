...
![](images/image-4836.jpg)

生成了 .env 和 schema 文件：

![](images/image-4837.jpg)

然后改下 .env 文件的数据库连接信息：

![](images/image-4838.jpg)
...
![](images/image-4840.jpg)

这个生成位置是可以改的：

![](images/image-4841.jpg)
...
![](images/image-4842.jpg)

![](images/image-4843.jpg)
...
![](images/image-4855.jpg)

然后创建一个 aaa 的迁移：

![](images/image-4856.jpg)

它会创建对应的 sql 文件并执行：

![](images/image-4857.jpg)

![](images/image-4858.jpg)
...
![](images/image-4860.jpg)

我们再创建一个 model

![](images/image-4861.jpg)
...
![](images/image-4862.jpg)

![](images/image-4863.jpg)
...
![](images/image-4864.jpg)
生成的 sql 如下：

![](images/image-4865.jpg)
...
![](images/image-4866.jpg)

可以看到 @db.xxx、@map、@@map、@@index 都生效了

在 mysql workbench 里也可以看到这个表：

![](images/image-4867.jpg)

那表和表之间的一对多、多对多关系呢？

我们再添加几个 model：

![](images/image-4868.jpg)
...
![](images/image-4869.jpg)

可以看到，生成的 sql 是符合预期的：

![](images/image-4870.jpg)

并且在数据库里可以看到创建了对应的外键：

![](images/image-4871.jpg)

然后是多对多：

![](images/image-4872.jpg)
...
![](images/image-4873.jpg)

生成的 sql 如下：

![](images/image-4874.jpg)

中间表创建了 postId 和 tagId 的联合主键，并且创建了两个外键。

在 mysql 里可以看到这两个外键：

![](images/image-4875.jpg)

这就是一对多、多对多的映射方式。

至于一对一，那个就是在多的那一侧添加一个 unique 约束就好了：

![](images/image-4876.jpg)

这样就把一对多变成了一对一。

此外，还有一个 enum 的语法，就是可以指定某个字段只能取一些枚举值：

![](images/image-4877.jpg)
...
![](images/image-4878.jpg)

生成的 sql 如下：

![](images/image-4879.jpg)

这个 enum 也是 sql 支持的语法（我们前面貌似没用过）。

![](images/image-4880.jpg)
...
![](images/image-4881.jpg)

每一个表的字段还有可用的 CRUD 方法都列出来了：

![](images/image-4882.jpg)
