...
![](images/image-2563.jpg)

但是都没有 param 的描述和 response 的描述：

![](images/image-2564.jpg)
...
![](images/image-2565.jpg)

再加上 @ApiQuery 来添加 query 参数的说明：

![](images/image-2566.jpg)
...
![](images/image-2572.jpg)

那如果 id 不合法的时候，我返回了一个 401 的响应呢？

那就再加一个 @ApiResponse

![](images/image-2573.jpg)
...
![](images/image-2574.jpg)

然后再来写 ccc 接口的文档：

ccc 接收的是请求体的参数，我们创建个 CccDto。

![](images/image-2575.jpg)
...
![](images/image-2577.jpg)

在 Nest 里要能清楚的区分 dto、vo、entity 的区别：

![](images/image-2578.jpg)
...
![](images/image-2582.jpg)

其中 bbb 是没有 \* 的，代表可不传。

上面的接口部分也有了请求和响应的示例：

![](images/image-2583.jpg)

点下 try it out，可以自己编辑请求体：

![](images/image-2584.jpg)

然后点击 execute 就可以看到响应的内容和 header：

![](images/image-2585.jpg)

服务端也确实接收到了这个请求：

![](images/image-2586.jpg)

可以用 swagger 来方便的测试接口。

此外，还可以通过 @ApiTags 来给接口分组：

![](images/image-2587.jpg)

比如 controller 是 xxx 开头的，那可以用 @ApiTags 来分组到 xxx。

显示的时候，就会把这个 controller 的接口分到一个组下：

![](images/image-2588.jpg)

也可以添加在 handler 上：

![](images/image-2589.jpg)

![](images/image-2590.jpg)

比如把 aaa、bbb 接口分到 xxx-get 的组。

那显示的时候就会把这俩接口分出来：

![](images/image-2591.jpg)
...
![](images/image-2593.jpg)

![](images/image-2594.jpg)

![](images/image-2595.jpg)

然后在 main.ts 里添 3 种认证方式的信息：

![](images/image-2596.jpg)
...
![](images/image-2605.jpg)

装个格式化 chrome 插件格式化一下，大概是这样的：

![](images/image-2606.jpg)
