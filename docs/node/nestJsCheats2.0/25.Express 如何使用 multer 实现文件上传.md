...
![](images/image-736.jpg)

浏览器访问下：

![](images/image-737.jpg)

这时候在 devtools 可以看到 aaa 请求的 body 是多个 boundary 分隔的格式：

![](images/image-738.jpg)

而分隔符是在 Content-Type 指定的：

![](images/image-739.jpg)

这就是 form-data 的传输格式。

然后去服务端看看：

![](images/image-740.jpg)

req.file 可以拿到文件字段，其余非文件字段在 req.body。

并且服务端多了 uploads 目录，下面就保存着我们上传的文件：

![](images/image-741.jpg)

单文件上传我们会了，那多文件上传呢？

再添加一个路由处理多文件的上传：

![](images/image-742.jpg)
...
![](images/image-743.jpg)

node 服务的 req.files 接收到了多个文件：

![](images/image-744.jpg)

并且 uploads 目录下也多了俩文件：

![](images/image-745.jpg)
...
![](images/image-747.jpg)

可以看到服务端打印了报错：

![](images/image-748.jpg)
...
![](images/image-749.jpg)

![](images/image-750.jpg)
...
![](images/image-751.jpg)

服务端收到了 aaa 和 bbb 的文件：

![](images/image-752.jpg)
...
![](images/image-755.jpg)

我们用时间戳 Date.now() 加上 Math.random() 乘以 10 的 9 次方，然后取整，之后加上原来的文件名。

测试下：

![](images/image-756.jpg)

然后浏览器再次上传：

![](images/image-757.jpg)

就可以看到目录和文件名都修改了：
![](images/image-758.jpg)
