...
![](images/image-3166.jpg)

可以看到，触发了下载。

在 devtools 里可以看到正确设置了 header：

![](images/image-3167.jpg)
...
![](images/image-3170.jpg)

![](images/image-3171.jpg)
...
![](images/image-3173.jpg)

![](images/image-3174.jpg)

效果一样。

只是这里的 Content-Type 默认是 application/octet-stream 二进制流：

![](images/image-3175.jpg)
...
![](images/image-3177.jpg)

安装后把它跑起来：

![](images/image-3178.jpg)

选择 loopback 这个网卡，本地回环地址，可以抓到 localhost 的包：

![](images/image-3179.jpg)

输入过滤器 port 3000，也就是过滤 3000 端口的数据包。

然后回车就会进入抓包界面：

![](images/image-3180.jpg)
...
![](images/image-3182.jpg)

这两个分别是请求和响应：

![](images/image-3183.jpg)

如果多了找不到，点一下 protocal 会排序：
![](images/image-3184.jpg)

可以看到，确实是分块传输的：

![](images/image-3185.jpg)

这里有一个数据块，然后一个空块结尾。

再访问下 http://localhost:3000/download 接口对比下：

![](images/image-3186.jpg)

和上面的对比下，这就是没有分块的响应。

当然，现在的文件比较小，可以找一个大一点的文件试一下：

![](images/image-3187.jpg)

可以看到，现在分片就多了：

![](images/image-3188.jpg)

大概是 65536 一个分块，也就是 64k。

每个分块都有 chunk size 和 chunk data：

![](images/image-3189.jpg)
